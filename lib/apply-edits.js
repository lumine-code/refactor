...
class RenameResponse {
  ...
  relativizePath(filePath) {
    const [, relative] = atom.project.relativizePath(filePath);
    return relative;
  }
...
const ApplyEdits = {
  ...
  findEditorForPath(filePath) {
    return atom.workspace.getTextEditors().find((editor) => editor.getPath() === filePath) ?? null;
  },
  ...
  shouldSaveEditor(editor) {
    ...
    return scope
      ? atom.config.get("refactor.saveAfterEditInOpenBuffers", { scope: [scope] })
      : atom.config.get("refactor.saveAfterEditInOpenBuffers");
  },
